...
for line in sys.stdin:
 ...
 time.sleep(.15)
 print(json.dumps({'type':'result','id':r['id'],'matches':[{'id':rows[0]['id'],'score':.8}]}),flush=True)
''')
...
ShellRoot {
 ...
 Timer { interval: 25; repeat: true; running: true; onTriggered: {
   if(test.stage===0) { session.submit("old","old",[{id:"old",text:"old"}]); test.stage=1 }
     ...
     session.enabled=false; test.stage=6
   } else if(test.stage===6 && !session.stopping) { console.log("PASS matching lifecycle"); Qt.quit() }
 } }
