...
- **Fuzzy everywhere, into submenus.** From the root, `filmod`, `nixsefimo` and `setfilmo` all land on Settings › Files › Search in the main palette, `sealit` on its Literal choice, `sysshut` on System › Shutdown. Letters may skip whole words of the breadcrumb, words can come in any order (`fil mode`), descriptions match by word. Inside a submenu the same search covers everything below it.
- **Answers first.** Computed results appear as answer rows with a preview; matches next; Nixi, your coding agent and Google last.
...
- **Files and folders** under `~` join the results from two characters on. Fuzzy abbreviations work: `dwnlds` finds Downloads, and `~dcmnts rpt` searches only files and folders for reports under Documents. `~` always selects fuzzy file search; Settings → Files → **Search in the main palette** offers **Fuzzy** (default), **Literal**, or **Starts with ~**. Hidden entries are optional; gitignore rules remain respected. `↵` opens the result; `Ctrl+↵` opens a terminal there. At most ten results mix into the main palette; `~` and the Files screen show up to sixty.
...
- **Hand off to your coding agent.** Any query offers **Ask \<default agent\>**: the agent picked in Omarchy's Setup › Default Agent (Claude Code, Codex, OpenCode, Gemini CLI, GitHub Copilot, Crush, Grok, Pi and the rest `omarchy agent` knows) opens in a terminal with your text as its prompt, passed as a literal argument through `omarchy-agent-prompt`. The agent starts with automatic approval and can run commands without asking, so `↵` always shows a confirmation that names the agent and repeats your text first. With no default agent the row opens the Default Agent picker; an agent that is not installed is never launched. Antigravity opens without your text. With the Nixi plugin enabled, **Ask Nixi** comes first and asks Nixi, which asks before it runs anything (a Nixi without `nixi --ask` opens with your question on the clipboard instead); start the query with `?` (`? how do I add a printer`) to lift Ask Nixi to the top with the answers. **Search Google** is always last.
...
The bundled providers live in `providers/`: Omarchy menu, applications, open URL, calculator, converter, colors, emoji, clipboard, files, hotkeys, agent and web hand-offs, and settings. The provider contract, shared by extensions, is [docs/providers.md](docs/providers.md); [docs/architecture.md](docs/architecture.md) describes the design.

**Upgrading from the Codex client.** The in-palette Codex conversation was removed in favour of the agent hand-off. It left `~/.local/state/nixarchy-menu/codex.json` and `~/.local/state/nixarchy-menu/questions/` behind; nothing reads them any more, and they can be deleted.
...
agent prompts retain the original transcript. Results still require Enter and
